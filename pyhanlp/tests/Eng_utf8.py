...
def GetAbstract(FileList):
    content = pd.concat([pd.read_table(name, encoding="utf-8",engine='python') for name in FileList], axis=1)#读取待分类文件及内容
    content=content.T
...
def GetLabelList(LabelPath):#输入包含标签的文件绝对路径
    f = open(LabelPath, encoding="utf-8")
    LabelList = []
...
def Run_Eng():
    ...
    FilePath = 'D:\\papers_classification\\pyhanlp\\tests\\categories_test\\test2'
